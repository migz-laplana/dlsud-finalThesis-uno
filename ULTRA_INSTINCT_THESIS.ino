...
void loop()
{
  ...
  while (hasDriver == 0)
  {
    ...
    if (Serial.available() > 0)
    {
      ...
      { //if driverid cant be found in database
      ...
      else if (data.indexOf("Accepted") > 0)
      {
        ...
        Serial.flush();
        hasDriver = 1;
        Serial.print("Driverid registered: ");
  ...
  while (sigPass == 0)
  { //make a signal-availability check
    ...
    while (ss.available() > 0)
      if (gps.encode(ss.read()))
        ...
        {
          Serial.println("No signal, Searching for signal..");
          delay(2000);
        }
  }
  ...
  if (content.substring(1) == driverid) //if driver signs off while prototype is running
  {
    hasDriver = 0;
    driverid = "";
    Serial.println("Driver logged out.");
    delay(3000);
    return;
  }
  ...
  for (int x = 0; x < 5; x++) //CHECK NATIN IF NAKASAKAY NA SIYA
  {
    if (person[x] == content.substring(1) && readerActive == 2 && !wrongReader)
    {
      ...
      while (ss.available() > 0)
      {
        ...
        Serial.print(lat2[x], 6);
        Serial.print("///");
        Serial.print(long2[x], 6);

        Serial.println();
        break;
      }
      Serial.print("byebye"); //send a signal to rpi that this is the passenger's second tap
      Serial.print(lat1[x], 6);
      Serial.print("#");
      Serial.print(long1[x], 6);
      Serial.print("#");
      Serial.print(lat2[x], 6);
      Serial.print("#");
      Serial.print(long2[x], 6);
  ...
  for (int x = 0; x < 5; x++)
  {
    if (person[x] == "" && readerActive == 1 && !wrongReader2) //check for free slots
    {
      ...
      if (Serial.available() > 0)
      {
        ...
        { //unsave passenger if invalid passenger
          occupiedSlots -= 1;
          Serial.print("Occupied seats: ");
          Serial.println(occupiedSlots);
          person[x] = "";
